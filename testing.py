...
def cardEditor():
    ...
    # Uses cont1 variable since we are working with nested loops
        ...
        while cont1:
            ...
            grade = grades(avg)
            card = f"---------------------\n{name}'s report card:\nmaths marks: {math}\ncomputer marks: {cs}\nenglish marks: {eng}\nTotal marks: {total}\nAverage: {avg}\nGrade: {grade}\n---------------------\n"
            pickle.dump(card,b,pickle.HIGHEST_PROTOCOL)
            b.close()
            temp = ra('Class Card')
            for i in range(len(temp)):
                if temp[i] == f"{name}'s report card:\n":
                    ...
                    pickle.dump(temp,a,pickle.HIGHEST_PROTOCOL)
                    a.close()
            # choice
            while True:
        ...
        while True:
            ...
            try:
                with open(f"{name} report card", "rb") as f:  # making it easier to work with.
                    report_card = pickle.load(f)  # reads the report card and stores it into a variable
